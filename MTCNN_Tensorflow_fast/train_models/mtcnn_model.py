...
def shufflenet_v2_block(x, num_outputs, kernel_size, stride=1, dilation=1, shuffle_group=2):
    with tf.variable_scope(None, 'shuffle_v2_block'):
        if stride == 1:
            top, bottom = tf.split(x, num_or_size_splits=2, axis=3)

            half_channel = num_outputs // 2
...
def conv_bn_relu(x, out_channel, kernel_size, stride=1, dilation=1):
    with tf.variable_scope(None, 'conv_bn_relu'):
        ...
        x = slim.batch_norm(x, activation_fn=prelu, fused=False)
    return x
...
def depthwise_conv_bn(x, kernel_size, stride=1, dilation=1):
    with tf.variable_scope(None, 'depthwise_conv_bn'):
        x = slim.separable_conv2d(x, None, kernel_size, padding = 'same', depth_multiplier=1, stride=stride,
...
def P_Net(inputs,label=None,bbox_target=None,landmark_target=None,training=True):
    ...
    with slim.arg_scope([slim.conv2d],
        ...
        net = slim.conv2d(inputs, num_outputs=10, kernel_size=[3,3], stride=2,scope='conv1')
        print (net.get_shape())
...
def R_Net(inputs,label=None,bbox_target=None,landmark_target=None,training=True):
    with slim.arg_scope([slim.conv2d],
                        ...
                        padding='same'):
        print (inputs.get_shape())
        net = slim.conv2d(inputs, num_outputs=28, kernel_size=[3,3], stride=2, scope="conv1")
        print (net.get_shape())
        #net = slim.max_pool2d(net, kernel_size=[3, 3], stride=2, scope="pool1", padding='SAME')
        #print (net.get_shape())
        net = slim.conv2d(net,num_outputs=48,kernel_size=[3,3],stride=2,scope="conv2")
        print (net.get_shape())      
        #net = slim.max_pool2d(net,kernel_size=[3,3],stride=2,scope="pool2")
        #print (net.get_shape())
        net = slim.conv2d(net,num_outputs=64,kernel_size=[3,3],stride=2,scope="conv3")
        print (net.get_shape())        
...
def O_Net(inputs,label=None,bbox_target=None,landmark_target=None,training=True):
    with slim.arg_scope([slim.conv2d],
        ...
        #net = slim.max_pool2d(net, kernel_size=[3, 3], stride=2, scope="pool1", padding='SAME')
        #print (net.get_shape())
        net = slim.conv2d(net,num_outputs=64,kernel_size=[3,3],stride=2,scope="conv2")
        print (net.get_shape())
        #net = slim.max_pool2d(net, kernel_size=[3, 3], stride=2, scope="pool2")
        #print (net.get_shape())
        net = slim.conv2d(net,num_outputs=64,kernel_size=[3,3],stride=2,scope="conv3")
